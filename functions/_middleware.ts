...
export const onRequest: PagesFunction = async (context) => {
  ...
  if (hasCard) {
    ...
    if (card.name)    imageParams.set('name',    card.name);
    if (card.nameEn)  imageParams.set('nameEn',  card.nameEn);
    if (card.title)   imageParams.set('title',   card.title);
    if (card.company) imageParams.set('company', card.company);
    ogImageUrl = `${SITE_URL}/api/og?${imageParams.toString()}`;
  }
